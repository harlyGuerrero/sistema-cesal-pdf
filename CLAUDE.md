...
4. **Product no pertenece a un solo Import**: la relación es `Product ← ImportItem ← Import`, nunca `Product.importId` obligatorio.
5. **Categorías patrimoniales cerradas**: exactamente 6 (`EQUIPOS_INFORMATICOS`, `EQUIPOS_DE_OFICINA`, `MUEBLES_DE_OFICINA`, `BIENES_VEHICULARES`, `EQUIPOS_DE_MAQUINARIA`, `BIENES_INMUEBLES`). Nunca crear `OTROS`. La relevancia (`PRODUCT`/`CONSUMABLE`/`SERVICE`/`OTHER`) es un concepto distinto y no es una categoría patrimonial.
